...
# 모듈 이름과 버전, 스크립트 이름을 정의합니다.
MODULE_NAME = "binwalk"
MODULE_VERSION = "2.3.3"
SCRIPT_NAME = MODULE_NAME
MODULE_DIRECTORY = os.path.dirname(os.path.realpath(__file__))

# Python3에서는 기본적으로 DEVNULL을 지원하지만, Python2에서는 수동으로 설정해야 합니다.
try:
    from subprocess import DEVNULL
except ImportError:
    DEVNULL = open(os.devnull, 'wb')

# git 리포지토리에서 체크아웃된 버전이라면, git 커밋 해시를 버전 번호에 포함시킵니다.
try:
...
# Python2/3 호환성을 위한 raw_input 처리
try:
    raw_input
except NameError:
    raw_input = input
...
# 지정된 명령어의 경로를 찾습니다.
def which(command):
    # /usr/local/bin은 일반적인 기본 설치 경로이지만, $PATH에 없을 수 있습니다.
...
class TestCommand(Command):
    ...
    def run(self):
        # 테스트를 위해 nose 모듈이 필요합니다.
        import nose

        # 테스트 디렉토리로 이동합니다. 그렇지 않으면 nose가 src/binwalk 디렉토리를
        # 가져와서 문제를 일으킬 수 있습니다.
